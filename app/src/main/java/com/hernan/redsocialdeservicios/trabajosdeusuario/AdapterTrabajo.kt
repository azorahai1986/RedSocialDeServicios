...
class AdapterTrabajo(var listTrabajo:ArrayList<ModeloTrabajos>, val fragment:FragmentActivity):
    ...
    override fun onBindViewHolder(holder: ViewHolderTrabajo, position: Int) {
        ...
        Glide.with(fragment).load(trabajosList.imageUsuario).into(holder.binding.imagenItemTrabajo)
        Glide.with(fragment).load(trabajosList.imageUsuario).into(holder.binding.imagenUsuarioTrabajo)
    }
